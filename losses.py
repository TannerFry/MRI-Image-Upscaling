...
def style_loss(x, target):
    ...
    y_gram = gram_matrix(target)
    return tf.reduce_sum(tf.square(y_gram - x_gram), axis=(1, 2))
...
def get_style_losses(outputs_dict, targets_dict, style_layers,
    ...
    '''
    return [style_loss(outputs_dict[l], targets_dict[l],norm_by_channels=norm_by_channels) for l in style_layers]
